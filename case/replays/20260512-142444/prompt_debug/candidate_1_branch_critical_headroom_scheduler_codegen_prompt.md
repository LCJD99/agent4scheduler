  ...
  "trace_findings": [
    {
      ...
      "natural_language_advice": "Investigate CPU scheduling policies to prioritize critical tasks over agent tasks. Consider isolating CPU cores for critical nodes or adjusting the scheduling priority of navigation_algo_node to ensure it can meet its deadlines. Also, review the execution time of image_captioning to reduce its CPU footprint during critical periods.",
      "root_cause_hypothesis": "The navigation_algo_node misses deadlines due to CPU contention from a concurrently running agent task (image_captioning), which runs at high CPU utilization (up to 94.75%) and may monopolize CPU resources, preventing the critical navigation node from meeting its timing constraints.",
      "severity": "high"
    }
  ],
  "workload_signature": {
